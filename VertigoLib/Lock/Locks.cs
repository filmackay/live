...
namespace Vertigo
{
    ...
    class DdMonitor
    {
        ...
        public static IDisposable Lock(object monitor)
        {
            ...
            Enter(monitor);
            return Disposable.Create(() => Exit(monitor));
        }
        ...
        public static bool TryEnter(object monitor, int millisecondsTimeout)
        {
            ...
            try
            {
                ...
                if (millisecondsTimeout == -1)
                {
                    while (true)
                    {
                        thisThreadOwnsMonitor = Monitor.TryEnter(monitor, 1000);
                        if (thisThreadOwnsMonitor)
                            break;
                        if (ms.OwningThread != null)
                        {
                            Debug.Print("Blocked thread: #{0} waiting for #{1}\n{2}", Thread.CurrentThread.ManagedThreadId, ms.OwningThread.ManagedThreadId, ms.OwnerStack);
                        }
                    }
                }
                else
                    ...
                    if (ms != null) // This would only be null if an exception occurred at a really weird place
                    {
                        ...
                        if (thisThreadOwnsMonitor)
                        {
                            if (ms.OwningThread != Thread.CurrentThread)
                            {
                                ...
                                ms.OwnerStack = new StackTrace(true);
                            }
    ...
    public delegate IDisposable Locker(bool block = true);

    public static class Lockers
    {
        public static Locker Empty = block => Disposable.Empty;
    }

    public static class Locks
    {
        // locker that returns no lock
        public static Locker EmptyLocker = block => null;
        public static HashSet<object> Track = new HashSet<object>();
    }

    public static partial class Extensions
    {
#if DEBUG_DEADLOCKS
        public static IDisposable Lock(this object obj, bool block = true)
        {
            if (obj == null)
                return Disposable.Empty;
            if (block)
                DdMonitor.Enter(obj);
            else if (!DdMonitor.TryEnter(obj))
                return null;
            return Disposable.Create(() => DdMonitor.Exit(obj));
        }
#else

#if DEBUG_TRACE_LOCKS
        static Extensions()
        {
            var file = File.CreateText(@"c:\temp\out.txt");
            file.AutoFlush = true;
            _out = file;
        }

        private readonly static TextWriter _out;
#endif

        public static IDisposable Lock(this SpinLock obj, bool block = true)
        {
#if DEBUG_TRACE_LOCKS
            var hashCode = obj.GetHashCode();
            var threadId = Thread.CurrentThread.ManagedThreadId;
            var str = string.Format("#{0} {1} {2} ", hashCode, threadId, obj.GetType());
            var str2 = str + "lock";
            lock (_out)
                _out.WriteLine(str2);
            if (Locks.Track.Contains(obj))
        ...
        public static Locker MergeLockers(this IList<Locker> items)
        {
            return block => items.LockAll(block);
        }

        public static IObservable<TSource> SynchronizeEx<TSource>(this IObservable<TSource> source, object gate)
        {
#if DEBUG_TRACE_LOCKS
        ...
        public static CompositeDisposable LockAll(this IList<Locker> lockers, bool block = true)
        {
            var locks = lockers.LockAll2(block);
            return locks == null ? null : new CompositeDisposable(locks);
        }

        public static IDisposable[] LockAll2(this IList<Locker> lockers, bool block = true)
        {
            // obtains a read lock on all items
            if (lockers.Count == 0)
                return null;
            var locks = new IDisposable[lockers.Count];
            var firstIndex = 0;      // first lock to try obtaining

        start:
            // get first lock
            locks[firstIndex] = lockers[firstIndex](block);
            if (locks[firstIndex] == null)
                return null;

            // get remaining locks, until we are unable to
            for (var j = 1; j < lockers.Count; j++)
            {
                var index = (firstIndex + j) % lockers.Count;
                locks[index] = lockers[index](false);
                if (locks[index] == null)
                {
                    // couldn't get them all - release all locks
                    for (var i = 0; i < j; i++)
                        locks[(firstIndex + i) % lockers.Count].Dispose();
        ...
        public static IDisposable WriteLock(this ReaderWriterLockSlim @lock, bool block = true)
        {
            ...
            //}
            //var threadID = Thread.CurrentThread.ManagedThreadId;
            //var hashCode = @lock.GetHashCode();
            //Debug.Print("==>> Enter {0} {1} {2}", hashCode, threadID, id);
            if (block)
                @lock.EnterWriteLock();
            ...
            return Disposable.Create(() =>
            {
                //Debug.Print("<<== Exit {0} {1} {2}", hashCode, Thread.CurrentThread.ManagedThreadId, id);
                @lock.ExitWriteLock();
            });
        }

        //public static IDisposable[] UnpackComposite(this IDisposable disposable)
        //{
        //    var composite = disposable as CompositeDisposable;
        //    if (composite == null)
        //        throw new InvalidOperationException("Lock is not composite");
